  ...
  describe "initialize" do
    ...
    context "the user trys to access an existing project" do
      it "does not raise an error" do
        expect { CreateCommit.new(project: project, branch: branch, comments: "",  notes:[]).call }.to_not raise_error
      end
    end
    ...
    context "a commit is saved to the db" do
      it "increases the commit count by one" do
        expect {
          CreateCommit.new(project: project, branch: branch, comments: "", notes:[]).call
